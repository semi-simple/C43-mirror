...
void longIntegerMultiply(longInteger_t opY, longInteger_t opX, longInteger_t result) {
  if(longIntegerBits(opY) + longIntegerBits(opX) <= MAX_LONG_INTEGER_SIZE_IN_BITS) {
    ...
    #if(EXTRA_INFO_ON_CALC_ERROR == 1)
      sprintf(errorMessage, "Multiplying this 2 values (%" PRIu64 " bits " STD_CROSS " %" PRIu64 " bits) would result in a value exceeding %" PRId16 " bits!", longIntegerBits(opY), longIntegerBits(opX), MAX_LONG_INTEGER_SIZE_IN_BITS);
...
void longIntegerSquare(longInteger_t op, longInteger_t result) {
  if(longIntegerBits(op) * 2 <= MAX_LONG_INTEGER_SIZE_IN_BITS) {
    ...
    #if(EXTRA_INFO_ON_CALC_ERROR == 1)
      sprintf(errorMessage, "Squaring this value (%" PRIu64 " bits) would result in a value exceeding %" PRId16 " bits!", longIntegerBits(op), MAX_LONG_INTEGER_SIZE_IN_BITS);
...
void longIntegerAdd(longInteger_t opY, longInteger_t opX, longInteger_t result) {
  if(longIntegerSign(opY) != longIntegerSign(opX) || max(longIntegerBits(opY), longIntegerBits(opX)) <= MAX_LONG_INTEGER_SIZE_IN_BITS - 1) {
    ...
    #if(EXTRA_INFO_ON_CALC_ERROR == 1)
      sprintf(errorMessage, "Adding this 2 values (%" PRIu64 " bits " STD_CROSS " %" PRIu64 " bits) would result in a value exceeding %" PRId16 " bits!", longIntegerBits(opY), longIntegerBits(opX), MAX_LONG_INTEGER_SIZE_IN_BITS);
      longIntegerToAllocatedString(opY, tmpStr3000, TMP_STR_LENGTH / 2);
      longIntegerToAllocatedString(opX, tmpStr3000 + TMP_STR_LENGTH / 2, TMP_STR_LENGTH / 2);
...
void longIntegerSubtract(longInteger_t opY, longInteger_t opX, longInteger_t result) {
  if(longIntegerSign(opY) == longIntegerSign(opX) || max(longIntegerBits(opY), longIntegerBits(opX)) <= MAX_LONG_INTEGER_SIZE_IN_BITS - 1) {
    ...
    displayCalcErrorMessage(longIntegerSign(opY) == 0 ? ERROR_OVERFLOW_PLUS_INF : ERROR_OVERFLOW_MINUS_INF, ERR_REGISTER_LINE, REGISTER_X);
    #if(EXTRA_INFO_ON_CALC_ERROR == 1)
      sprintf(errorMessage, "Subtracting this 2 values (%" PRIu64 " bits " STD_CROSS " %" PRIu64 " bits) would result in a value exceeding %" PRId16 " bits!", longIntegerBits(opY), longIntegerBits(opX), MAX_LONG_INTEGER_SIZE_IN_BITS);
      longIntegerToAllocatedString(opY, tmpStr3000, TMP_STR_LENGTH / 2);
      longIntegerToAllocatedString(opX, tmpStr3000 + TMP_STR_LENGTH / 2, TMP_STR_LENGTH / 2);
