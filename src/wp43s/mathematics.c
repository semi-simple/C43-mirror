...
void fnLog2(uint16_t unusedParamButMandatory) {
  ...
  if(getRegisterDataType(REGISTER_X) == dtSmallInteger) {
    ...
    *(uint64_t *)(POINTER_TO_REGISTER_DATA(REGISTER_X)) = WP34S_intLog2(*(uint64_t *)(POINTER_TO_REGISTER_DATA(REGISTER_X)));
    ...
    copySourceRegisterToDestRegister(REGISTER_X, REGISTER_L);
    convertBigIntegerRegisterToBigInteger(REGISTER_X, &value);
    ...
    if(bigIntegerIsZero(&value) || signX) {
      ...
      #if (EXTRA_INFO_ON_CALC_ERROR == 1)
        showInfoDialog("In function fnLog2: Cannot calculate the log" STD_SUB_2 " of a number " STD_LESS_EQUAL " 0!", NULL, NULL, NULL);
      #endif
