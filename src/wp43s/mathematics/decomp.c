 ...
 * result = MOD(a, b) = a % b
 */
static void modReal(real39_t *a, real39_t *b, real39_t *result) {
 ...
 * result = GCD(x, y)
 */
static void gcdReal(real39_t *x, real39_t *y, real39_t *result) {
    real39_t b, t;
    ...
    while (!realIsZero(&b)) {       // b != 0
        realCopy(&b, &t);           // t = b;
        modReal(result, &t, &b);    // b = result mod t
