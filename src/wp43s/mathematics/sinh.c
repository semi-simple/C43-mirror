...
void sinhRema(void) {
  ...
  for(int i = 0; i < cols * rows; ++i) {
    ...
    if(realIsInfinite(&el)) {
      ...
      WP34S_SinhCosh(&el, &el, NULL, &ctxtReal39);
  ...
  if(fail) {
    ...
    #if (EXTRA_INFO_ON_CALC_ERROR == 1)
      moreInfoOnError("In function sinhRema:", "cannot use " STD_PLUS_MINUS STD_INFINITY " as element input of sinh when flag SPCRES is not set", NULL, NULL);
    #endif // (EXTRA_INFO_ON_CALC_ERROR == 1)
  }
  else {
