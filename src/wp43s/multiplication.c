...
void fnSquare(uint16_t unusedParamButMandatory) {
  #if (LOG_FUNCTIONS == 1)
    enteringFunction("fnSquare");
  #endif

  if(multiplication[getRegisterDataType(REGISTER_X)][getRegisterDataType(REGISTER_X)] != errorMul) {
    copySourceRegisterToDestRegister(REGISTER_X, REGISTER_L);

    if(getRegisterDataType(REGISTER_X) == dtReal16 || getRegisterDataType(REGISTER_X) == dtReal34 || getRegisterDataType(REGISTER_X) == dtComplex16 || getRegisterDataType(REGISTER_X) == dtComplex34) {
      result = REGISTER_X;
      op1    = allocateTemporaryRegister();
      op2    = allocateTemporaryRegister();
      ...
      multiplication[getRegisterDataType(op2)][getRegisterDataType(op1)]();
      ...
      convertBigIntegerRegisterToBigInteger(REGISTER_X, &iOp);
      bigIntegerSquare(&iOp, &iOp);
      convertBigIntegerToBigIntegerRegister(&iOp, REGISTER_X);
    }

    else if(getRegisterDataType(REGISTER_X) == dtSmallInteger) {
      *(uint64_t *)(POINTER_TO_REGISTER_DATA(REGISTER_X)) = WP34S_intSqr(*(uint64_t *)(POINTER_TO_REGISTER_DATA(REGISTER_X)));
    }

    else {
      displayCalcErrorMessage(24, REGISTER_T, REGISTER_X); // Invalid input data type for this operation
      #if (EXTRA_INFO_ON_CALC_ERROR == 1)
        sprintf(errorMessage, "%s cannot be squared!", getDataTypeName(getRegisterDataType(REGISTER_X), true, false));
        showInfoDialog("In function fnSquare:", errorMessage, NULL, NULL);
      #endif
    }
...
void fnCube(uint16_t unusedParamButMandatory) {
  ...
  if(multiplication[getRegisterDataType(REGISTER_X)][getRegisterDataType(REGISTER_X)] != errorMul) {
    ...
    if(getRegisterDataType(REGISTER_X) == dtReal16 || getRegisterDataType(REGISTER_X) == dtReal34 || getRegisterDataType(REGISTER_X) == dtComplex16 || getRegisterDataType(REGISTER_X) == dtComplex34) {
      ...
      convertBigIntegerRegisterToBigInteger(REGISTER_X, &iOp1);
      convertBigIntegerRegisterToBigInteger(REGISTER_X, &iOp2); // TODO: use memcpy here
      bigIntegerSquare(&iOp2, &iOp2);
      bigIntegerMultiply(&iOp1, &iOp2, &iOp1);
      convertBigIntegerToBigIntegerRegister(&iOp1, REGISTER_X);
    }

    else if(getRegisterDataType(REGISTER_X) == dtSmallInteger) {
      *(uint64_t *)(POINTER_TO_REGISTER_DATA(REGISTER_X)) = WP34S_intCube(*(uint64_t *)(POINTER_TO_REGISTER_DATA(REGISTER_X)));
    }
