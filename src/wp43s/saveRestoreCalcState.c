...
static uint32_t restore(void *buffer, uint32_t size, void *stream) {
  ...
  void saveCalc(void) {
    ...
    save(&tam.digit,                          sizeof(tam.digit),                          BACKUP);
    save(&tam.letteredRegister,               sizeof(tam.letteredRegister),               BACKUP);
  ...
  void restoreCalc(void) {
    ...
    if(backupVersion != BACKUP_VERSION || ramSize != RAM_SIZE) {
      ...
      restore(&tam.digit,                          sizeof(tam.digit),                          BACKUP);
      restore(&tam.letteredRegister,               sizeof(tam.letteredRegister),               BACKUP);
